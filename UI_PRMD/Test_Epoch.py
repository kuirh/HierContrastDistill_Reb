...
def val_epoch(network, val_loader,dataloader,device):
    ...
    with torch.no_grad():  # 在验证阶段不需要计算梯度
        for batch in val_loader:
            ...
            predictions = torch.from_numpy(dataloader.sc2.inverse_transform(predictions.cpu().numpy()))
            labels = torch.from_numpy(dataloader.sc2.inverse_transform(labels.cpu().numpy()))
